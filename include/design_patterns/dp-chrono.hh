...
#ifndef DESIGN_PATTERNS_CXX_DP_CHRONO_HH
#define DESIGN_PATTERNS_CXX_DP_CHRONO_HH
...
inline void unix_time(struct timespec *spec) {
    ...
    wintime -= dp::chrono::detail::w2ux;
    spec->tv_sec = wintime / dp::chrono::detail::exp7;
    spec->tv_nsec = wintime % dp::chrono::detail::exp7 * 100;
}
inline int clock_gettime(int, timespec *spec) {
    ...
    if (tps != tmp) {
        ...
        unix_time(&startspec);
        ticks2nano = (double) dp::chrono::detail::exp9 / tps;
    }
    ::QueryPerformanceCounter((LARGE_INTEGER *) &curticks);
    curticks -= startticks;
    spec->tv_sec = startspec.tv_sec + (curticks / tps);
    spec->tv_nsec = (long) (startspec.tv_nsec + (double) (curticks % tps) * ticks2nano);
    if (!(spec->tv_nsec < dp::chrono::detail::exp9)) {
        spec->tv_sec++;
        spec->tv_nsec -= dp::chrono::detail::exp9;
    }
    return 0;
}
...
namespace dp::chrono {
     ...
     * @code{c++}
     *    void yours(){
     *          dp::chrono::high_res_duration hrd;
     *          
     *          //...
     *          
     ...
     * sample:
     * @code{c++}
     *     dp::chrono::high_res_duration hrd([](auto duration) -> bool {
    ...
    public:
        ...
        template<typename T,
                 std::enable_if_t<dp::chrono::is_duration<T>::value, bool> = true>
...
inline std::ostream &operator<<(std::ostream &os, const dp::chrono::iom::fmtflags v) {
    using iom = dp::chrono::iom;
    iom::set_flags(v);
    return os;
}
...
namespace dp::chrono {
     ...
     * if (dp::chrono::try_parse_by(tm, time_str, "%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M:%S")) {
     *     auto tp = dp::chrono::tm_2_time_point(&tm);
     *     // ...
     * }
     * @endcode
     */
...
inline std::ostream &operator<<(std::ostream &os, T const &v) {
    return dp::chrono::format_duration(os, v);
}

template<typename T,
         std::enable_if_t<dp::chrono::is_duration<T>::value, bool>>
inline void dp::chrono::high_res_duration::print_duration(std::ostream &os, T v) {
    os << "It took ";
    dp::chrono::format_duration(os, v);
    os << '\n';
}

template<class _Clock, class _Duration = typename _Clock::duration>
inline std::ostream &operator<<(std::ostream &os, std::chrono::time_point<_Clock, _Duration> const &time) {
    // std::size_t ns = dp::chrono::time_point_get_ns(time);
    return dp::chrono::serialize_time_point(os, time, "%F %T");
}

inline std::ostream &operator<<(std::ostream &os, std::tm const *tm) {
    return dp::chrono::serialize_tm(os, tm, "%F %T");
}
inline std::ostream &operator<<(std::ostream &os, std::tm const &tm) {
    return dp::chrono::serialize_tm(os, &tm, "%F %T");
}

inline std::ostream &operator<<(std::ostream &os, dp::chrono::clock const &v) {
    return v.serialize(os, "%F %T");
}


#endif //DESIGN_PATTERNS_CXX_DP_CHRONO_HH
