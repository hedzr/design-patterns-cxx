...
namespace dp::observer::basic {
    ...
    class Store : public dp::util::observable<event, true> {};

    class Customer : public dp::util::observer<event> {
    public:
        virtual ~Customer() {}
...
void test_observer_basic() {
    using namespace dp::observer::basic;
    ...
    dp::util::registerer<event, true> __r(store, c);
...
void test_util_bind() {
    using namespace std::placeholders;
    {
        ...
        helpers::Callback(dp::traits::l2f([](int a, float b) { std::cout << a << ',' << b << '\n'; }));

        // hicc::util::bind

        auto fn0 = dp::util::bind([](int a, float b) { std::cout << "\nfn0: " << a << ',' << b << '\n'; }, _1, _2);
        fn0(1, 20.f);

#ifndef _MSC_VER
        moo m;
        auto fn1 = dp::util::bind(&moo::doit, m, _1, 3.0f);
        std::cout << "fn1: " << fn1(1) << '\n';

        auto fn2 = dp::util::bind(doit, _1, 3.0f);
        std::cout << "fn2: " << fn2(9) << '\n';
#else
    ...
    {
        auto fn = dp::traits::lambda([&](std::vector<std::string> const &vec) {
            std::cout << vec << '\n';
        });
        std::vector<std::string> vec{"aa", "bb", "cc"};
        std::function<void(std::vector<std::string> const &)> x = fn;
        x(vec);
        dbg_print("lambda fn: %s", std::string(dp::debug::type_name<decltype(x)>()).c_str());
    }
}

namespace dp::observer::cb {
    ...
    class Store : public dp::util::observable_bindable<event> {};

} // namespace dp::observer::cb

void fntest(dp::observer::cb::event const &e) {
    dbg_print("event CB regular function: %s", e.to_string().c_str());
}

void test_observer_cb() {
    using namespace dp::observer::cb;
    using namespace std::placeholders;
    ...
    dbg_print("lambda: %s", std::string(dp::debug::type_name<decltype(fn)>()).c_str());
...
namespace dp::observer::slots::tests {
...
void test_observer_slots() {
    using namespace dp::observer::slots;
    using namespace dp::observer::slots::tests;
    using namespace std::placeholders;
    {
...
void test_observer_slots_args() {
    ...
    dp::util::signal<float, int, bool, std::string> sig;
...
int main() {
    ...
    DP_TEST_FOR(test_util_bind);

    DP_TEST_FOR(test_observer_basic);
    DP_TEST_FOR(test_observer_cb);
    DP_TEST_FOR(test_observer_slots);
    DP_TEST_FOR(test_observer_slots_args);
